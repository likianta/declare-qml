...
class DictTree:
    ...
    def insert_inside(self, key, **kwargs):
        ...
        ''' 为什么要用深拷贝?
            因为 self.default.values 可能含有 dict, list 等元素, 如果不采用深拷
            贝, 则这些元素的引用会被泄露到其他节点.
            
            注意: 请不要使用 `deepcopy(self.default | kwargs)`, 因为:
            1. 当 kwargs 结构比较复杂的时候, 深拷贝 kwargs 的成本非常高
            2. kwargs 不会产生引用泄露的问题, 所以不需要深拷贝
            3. 经测试, kwargs.values 中如果包含了 QtWidget 实例, 会导致深拷卡死
                在这一步
            
            PS: '|' 是 Python 3.9 新增的特性支持. 如果您的 Pycharm 在这里对 '|'
            提出警告, 说明您的 Pycharm 版本较低, 请升级 Pycharm 至 2020.03 及以
            上版本, 即可自动消除此警告.
        '''
